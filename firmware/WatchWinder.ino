...
void setup() {
  ...
  Serial.println("\n\n");
  Serial.println("Watch Winder FW V1.2 Cloud");
  Serial.println("--------------------------");
...
void loop() {
  ...
  // Winder run should not be impacted by other parts of the code.
  winder.run();

  if (winder.distanceToGo() == 0) {
    winder.disableOutputs();  // Stop motor to avoid overheating.
  }
}
